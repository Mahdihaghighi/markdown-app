...
async function seed() {
    ...
    try {
        ...
        await client.query("insert into users (username, password) values ($1,$2) on conflict do nothing", ["demo",hash]);

        const demoUserRes = await client.query("select id from users where username = 'demo'");
        const demoUser = demoUserRes.rows[0];
        for (let i = 0; i < 10; i++)
        {
            console.log(`create notes ${i} for demo user`);
            await client.query (
                "insert into notes (user_id, title, content) values ($1 ,$2 ,$3)",
                [demoUser.id, faker.lorem.sentence(), faker.lorem.paragraph()]
            )
        }
        ...
        const usersRes = await client.query(
            "select id from users order by create_at desc limit 10"
        )
